...
def neuroCombat(dat,
    ...
    s_data, s_mean, v_pool, mod_mean, B_hat = standardize_across_features(dat, design, info_dict)#***changed by xq
    ...
    estimates = {'batches': info_dict['batch_levels'], 'var.pooled': v_pool, 'stand.mean': s_mean, 'mod.mean': mod_mean, 'gamma.star': gamma_star, 'delta.star': delta_star,'beta.hat':B_hat}#***changed by xq
...
def standardize_across_features(X, design, info_dict):
    ...
    return s_data, stand_mean, var_pooled, mod_mean, B_hat#****changed by xq
...
def aprior(delta_hat):
    ...
    if s2 == 0:#***added by xq
        print("Error: Variance (s2) is zero. Stopping execution.")
