...
"""mod: covariates,
batch: categorical, batch effects from scanner,
"""
...
def it_sol(sdat, g_hat, d_hat, g_bar, t2, a, b, conv=0.0001, robust=False):
    ...
    g_old = g_hat.copy()
    # g_old=np.repeat(g_old,len(d_hat))
    d_old = d_hat.copy()
...
def getStandardizedDataDC(dat, data_dict, design, hasNAs, central_out):
    ...
    print(f"s_data NaN count: {np.isnan(s_data).sum().sum()}")
    return {
        "s_data": s_data,
...
def getNaiveEstimators(s_data, data_dict, hasNAs, mean_only, robust):
    ...
    if robust:
        ...
        if not hasNAs:
            gamma_hat = np.matmul(np.linalg.inv(np.matmul(batch_design.T, batch_design)), batch_design.T,)
            gamma_hat = np.matmul(gamma_hat, s_data.T)
        else:
